...
define ( 'GAP', ', ');  //What to put between scrolling text blocks.
...
//Play text to matrix
function scroll_text ( $name, $outText, $blockMode, $arrDispConfig ) {
    global $fontList;
...
//Play text to matrix
function show_text ( $name, $outText, $blockMode, $arrDispConfig ) {
    global $fontList, $pause;
    ...
    foreach ( $outText as $msg ) {
        $arrDispConfig['Message'] = $msg;
        do_put("http://localhost/api/overlays/model/$name/text",  $arrDispConfig ); 
        sleep ( $pause );
    }
}
...
while (TRUE) {
    ...
    if ( $fppStatus['fppd'] ) {
        ...
        if ( $FM ) {
            if ( is_active ( $FM, $fmOverlay ) ) $outText[] = "FM: ON";
            else $outText[] = "FM: OFF";
        }

        if ( $proj ) {
            if ( is_active ( $proj, $projOverlay ) ) $outText[] = "Projector: ON";
            else $outText[] = "Projector: OFF";
        }
        ...
        if ( $info ) {
            if ( is_active( $info, $infoOverlay ) ) {
                ...
                if ( $infoOffCount > 2 ) $outText[] = "Info: OFF";
                else {
                    $infoOffCount++;
                    $outText[] = "Info: ON";            
                }
        ...
        if ( $playlist ) {
            $outText[] = "Playlist...";
            $outText[] = substr ( $playlist, 0, 15 );

            if ( $sequence = $fppStatus['current_sequence'] ) {
                $outText[] = "Sequence...";
                $outText[] = substr ( $sequence, 0, 15 );
                $outText[] = "Song...";
                $outText[] = substr ( $fppStatus['current_song'], 0, 15 );
            }
        }
        elseif ( $fppStatus['next_playlist']['start_time'] ) {
            $outText[] = "Sched. Playlist...";
            $outText[] = substr ( $fppStatus['next_playlist']['playlist'], 0, 15 );
            $outText[] = "will run: " . $fppStatus['next_playlist']['start_time'];
        }
        else $outText[] = "Nothing scheduled";
    }
    else
    {
        //FPP is not running!
        ...
        $outText[] = "ShowRunner: OFF";
    }

    if ( $pos == "Center" ) show_text ( $name, $outText, $blockMode, $arrDispConfig ); 
    else scroll_text ( $name, $outText, $blockMode, $arrDispConfig );
    sleep ($sleepTime);
}
?>