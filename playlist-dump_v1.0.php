 ...
 * It intentionally runs slowly, to avoid overloading the FPP REST API.
 * 
*/
...
$playlistName = rawurlencode ( $playlistName );
$json = do_get ( "http://localhost/api/playlist/$playlistName" );
...
foreach ( $json['mainPlaylist'] as $item ) {
    ...
    $arrPlaylist[$i]['Runtime'] = null;
    $arrPlaylist[$i]['Title'] = null;
    $arrPlaylist[$i]['Artist'] = null;
    $arrPlaylist[$i]['Album'] = null;
    ...
    $arrPlaylist[$i]['Runtime'] = $strTime;
    $arrPlaylist[$i]['Title'] = $songTitle;
    if ( $songArtist ) $arrPlaylist[$i]['Artist'] = $songArtist;
    if ( $songAlbum ) $arrPlaylist[$i]['Album'] = $songAlbum;
    $i++;
}
