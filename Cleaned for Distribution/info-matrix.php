 ...
 * info-matrix.php
 * by Wolf I. Butler
 * v. 1.5, Last Updated: 11/01/2021
 * (Added file output to use for Web site.)
 * 
 ...
 * 
 * This script shoudl be run in the background using UserCallbackHook.sh
 * Put it in the boot section so it only runs once on startup.
 * YOU MUST END IT WITH ' &' SO IT RUNS IN THE BACKGROUND, OR FPP WILL NO BOOT ANYMORE!
 * 
 * Like this:
 *    boot)		
...
define ( "GAP", "  -  " );
define ( 'PATH', '/home/fpp/media/upload/' );   //Path for the display output file used by Web site.
define ( 'INFO', 'info.txt');    //Output file used by Web site.
    ...
    if ( $fppStatus['fppd'] ) {
        ...
        if ( $lastFile != $mediaFile ) {
            ...
            if ( $songTitle ) {
                ...
                $outFile .= 'Now Playing: ' . $songTitle ;
                if ( $songArtist ) {
                    $outText .= ", By: " . $songArtist ;
                    $outFile .= ", By: " . $songArtist ;
                }
                if ( $songAlbum ) {
                    $outText .= ", Album: " . $songAlbum ;
                    $outFile .= ", Album: " . $songAlbum ;
                }
                $outText .= GAP;
                $outFile .= "\r";
            }
        ...
        if ( is_file ( $info ) ) {
            ...
            $outText = PREROLL . GAP . POSTROLL;   //If no file, display what we can.
            $outFile = PREROLL . "\n" . POSTROLL;   
        }
    }
    //Write out current status to file for display on Web site
    if ( $outFile ) file_put_contents ( PATH . INFO, $outFile );
    echo "$outFile\n";
    //Display outText:
